...
function genMenu() {
  ...
  for (it in this.items) {
    if (this.items[it].capt=="-") {
      ...
      smenu+="<tr id=\""+it+"\"><td class=\""+this.stlprf+"ctxsidebar\" align=\"center\" style=\"font-size:3px\">&nbsp;</td><td style=\"height:1px;vertical-align:center;padding-top:3px\"><div class=\"ctxseparator\">&nbsp;</div></td></tr>";
	  //--    
    } else {
  ...
  if (!orgEvent || orgEvent.toString().search(/orgEvent/gi) < 0) {
    ...
    var newEvent = function() { hideAllMenu();}
	//--
...
//alert('x:' + x + ' y:' + y); 
  ...
  //	
  var pOffset = getParentOffset(NlsGetElementById(this.mId))
  ctx.style.left = ctx.offsetLeft - pOffset[0];     
  ctx.style.top = ctx.offsetTop - pOffset[1];
...
function hideMenu() {
  ...
  // - dnn specific
  //if (g_DNNDDId != '') {document.getElementById(g_DNNDDId).style.position = 'relative';}	
  //---
...
//
//--
function hideItem(key, b) {
  var intKey = this.mId+"_"+key;
  setStyleHidden(NlsGetElementById(intKey), (b?"none":"block"));
}
function setStyleHidden(it, s) {
  if (it != null) {
	it.style.display=s;
  }
}
